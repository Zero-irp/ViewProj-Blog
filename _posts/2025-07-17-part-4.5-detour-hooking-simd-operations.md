---
layout: post
title: "Part 4.5: Detour Hooking to Optimize SIMD Operations"
date: 2025-07-17 00:00:00 +0530
permalink: /part-4.5-detour-hooking-simd-operations/
---
...
In the previous post, I already pointed out how the game tends to over-engineer certain SIMD operations, performing a Vec4 × Matrix4x4 to 
reconstruct data that’s already directly available. Now it’s time to actually optimize those sequences.  

Take this section from sub_7FBF10, for example. At first glance, it looks like a massive chain of _mm_shuffle_ps, _mm_mul_ps, and 
_mm_add_ps calls:
...
Just as the name suggests, detour hooking is a technique that forces a function to take a different route, you modify the machine code at any point of a function so that, 
...
jump instruction. When that patched area executes, the CPU follows your detour.  
...
Common pitfalls include creating dead ends, returning incorrectly, corrupting registers, misalignment of stack (ABI convention RSP % 16 == 0 when a call is executed)
...
We’re looking at this function and it’s flooded with redundant SIMD instructions. Most of them contribute to the final ViewProjection result just in 
a very overengineered way that takes away performance (marginally). But scattered in between, there are still useful SIMD ops we need to keep:  
...
This construction of v17 is important as this is the one that constructs ViewProjection matrix with the translation. This function is scattered 
among the overengineered SIMD ops.

### **Here’s the approach:**

- Match assembly to pseudocode: I’ll go through the disassembly and identify which SIMD instructions actually correspond to the math we care 
about. The rest is junk.
- Build a custom hook: In a new code cave, I’ll re-implement only the useful SIMD ops in the correct order. This way, 
the essential calculations survive but the redundant ops get skipped.
...
This way, we don’t touch every instruction one by one, we just surgically detour into our cleaner version and rejoin the original code later 
skipping all redundant operations.

![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/redundant_simd_assembly.png)
...
![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/detourhook_codecave.png)

Now, before diving in

I won’t be showing how to find code caves here, that is out of scope for this blog. Just know that in this case, I’ll be using a code cave that’s 
conveniently located close enough to our target function.

### **5 Byte Jmp hook vs 13 Byte Jmp hook**
...
If you can’t rely on code caves (maybe there aren’t any close enough), then you can allocate your own memory with something like 
VirtualAllocEx. But in that case, the E9 relative jump won’t cut it anymore unless you allocate between ±2 GB.
...
It takes more bytes (usually 12+ depending on the register used), but it can jump anywhere in 64-bit address space. So:
...
We will add this in our code-cave (Keep in mind a1 is rbp in this specific case):
...
We Replace the first 5 bytes at the target address with JMP <code_cave> (Relative Address).
...
After our optimized SIMD code executes, we jump back to the original function. The game continues execution exactly where it would have been after the matrix construction, but with 
the over-engineered SIMD code removed.
...
![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/ce_hook_showcase.png)

And our custom payload is seen in the code cave:  

![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/ce_codecave_showcase.png)

### Observations in game and memory:

With the hook active, the game runs normally, no crashes, no artifacting. This validates our assumption that the stripped-out 
SIMD instructions were truly redundant and removing them only avoided a tiny bit of wasted work:

**Matrix values remain identical with or without the hook**
![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/matrix_comp.png)

**In-Game Observation**
![ESP-Image1](/ViewProj-Blog/assets/images/part-4.5/game_runs.gif)

>While not strictly redundant (the instructions still contribute to the final result), this sequence is clearly overengineered. 
The same output could be achieved with far fewer operations, which is why we treat them as ‘redundant’ for the purpose of optimization.
 
*The performance gain here is so minuscule you’d honestly get more FPS by closing a calculator app running in the background. But it teaches a 
bigger lesson: systems don’t always do things the cleanest way, and compilers sure as hell aren’t infallible. Do not blindly trust compilers!*
